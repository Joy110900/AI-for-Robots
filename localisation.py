...
def localize(colours,measurement,motion,sr,pm): # sr is sensor right which is the error in sensor,; pm is the error in movement
    ''' 
    This function returns back the final probability of the robot to be in each cell.
    First the robot moves and then it senses the environment. 
    '''
...
def sense(p, color, Z, st):
    '''
    This functions senses the environment by the measurement given to the world and updates the probablity.
    st is the probability that the sensor value is right.
    '''
    aux = [[0.0 for row in range(len(p[0]))] for col in range(len(p))]
    sw = 1 - st #sensor wrong value
    t = 0.0
...
def move(p, U, pm):
    '''
    This function updates the probablity after the robot has moved. 
    pm is the probability that the robot moves. 
    '''
    aux = [[0.0 for row in range(len(p[0]))] for col in range(len(p))]
    ps = 1 - pm #if bot does not move it stays. Probabiltiy of that.
    for i in range(len(aux)):
        for j in range(len(aux[i])):
